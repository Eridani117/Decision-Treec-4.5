...
void LoadTrainData(DATA_SET &t, int c) {
  ...
  SUB_TRAIN_SET->cnt = c;
  SUB_TRAIN_SET->datas = (int *)malloc(sizeof(int) * c);
  SUB_TRAIN_SET->labels = t->labels;
}
...
double get_label_entropy(SUB_DATA_SET data, int id) {
  ...
  for (int i = 0; i < data->cnt; i++) {
    if (train_set->datas[data->datas[i]*784+id]== '0') {
      num[data->labels[i]]++;
      num_0++;
    }
  }
  ...
  for (int i = 0; i < data->cnt; i++) {
    if (train_set->datas[data->datas[i]*784+id] == '1') {
      num[data->labels[i]]++;
      num_1++;
    }
  }
...
double get_label_spilt_entropy(SUB_DATA_SET data, int id) {
  ...
  double info = 0.0;
  for (int i = 0; i < data->cnt; i++) {
    if (train_set->datas[id+ data->datas[i]*step] == '1')
      one++;
    else
      zero++;
  }
...
void spilt_data(DATA_SET data, int id, SUB_DATA_SET &zero, SUB_DATA_SET &one) {
  ...
  int p1 = 0;
  int p0 = 0;
  zero->datas = (int *)malloc(sizeof(int) * size0);
  one->datas = (int *)malloc(sizeof(int) * size1);
  zero->labels = (char *)malloc(sizeof(int) * size0);
  one->labels = (char *)malloc(sizeof(int) * size1);
...
void buildTree(TREE &root, SUB_DATA_SET data) {
  ...
  spilt_data(train_set, max_IGR_f, zero, one);
